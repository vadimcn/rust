...
//! DWARF frame unwind info reading.
...
pub unsafe fn find_landing_pad(lsda: *const u8, context: EHContext) -> Option<usize> {
    ...
    while reader.ptr < action_table {
        ...
        // Callsite table is sorted by cs_start, so if we've passed the ip, we may stop searching.
...
unsafe fn read_encoded_pointer(reader: &mut DwarfReader,
    ...
    result += match encoding & 0x70 {
        ...
        DW_EH_PE_textrel => context.text_start as usize,
        DW_EH_PE_datarel => context.data_start as usize,
        DW_EH_PE_funcrel => context.func_start as usize,
        _ => panic!()
    };
